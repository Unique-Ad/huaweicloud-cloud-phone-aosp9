...
class ComposerClientImpl : public Interface {
   public:
    ...
    Return<Error> updateActiveConfigSize(Display display, uint32_t width, uint32_t height) override {
        Error err = mHal->updateActiveConfigSize(display, width, height);
        return err;
    }
    ...
    Return<Error> eventControl(Display display, int32_t event, int32_t enabled) override {
        Error err = mHal->eventControl(display, event, enabled);
        return err;
    }
