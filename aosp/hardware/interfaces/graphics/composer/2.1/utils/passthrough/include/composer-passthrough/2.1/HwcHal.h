...
class HwcHalImpl : public Hal {
   public:
    ...
    Error updateActiveConfigSize(Display display, uint32_t width, uint32_t height) override {
        int32_t err = mDispatch.updateActiveConfigSize(mDevice, display, width, height);
        return static_cast<Error>(err);
    }
    ...
    Error eventControl(Display display, int32_t event, int32_t enabled) override {
        int32_t err = mDispatch.eventControl(mDevice, display, event, enabled);
        return static_cast<Error>(err);
    }
   ...
   protected:
    ...
    virtual bool initDispatch() {
        if (!initDispatch(HWC2_FUNCTION_ACCEPT_DISPLAY_CHANGES, &mDispatch.acceptDisplayChanges) ||
            ...
            !initDispatch(HWC2_FUNCTION_UPDATE_ACTIVE_CONFIG_SIZE, &mDispatch.updateActiveConfigSize) ||
            !initDispatch(HWC2_FUNCTION_SET_CLIENT_TARGET, &mDispatch.setClientTarget) ||
            !initDispatch(HWC2_FUNCTION_EVENT_CONTROL, &mDispatch.eventControl) ||
    ...
    struct {
        ...
        HWC2_PFN_UPDATE_ACTIVE_CONFIG_SIZE updateActiveConfigSize;
        ...
        HWC2_PFN_EVENT_CONTROL eventControl;
    } mDispatch = {};
