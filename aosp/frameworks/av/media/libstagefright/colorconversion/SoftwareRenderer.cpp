...
std::list<FrameRenderTracker::Info> SoftwareRenderer::render(
    ...
    void *dst;
    android_ycbcr dstYCbCr;
    if (mColorFormat == OMX_COLOR_FormatYUV420Planar) {
        CHECK_EQ(0, mapper.lockYCbCr(buf->handle,  GRALLOC_USAGE_SW_READ_NEVER | GRALLOC_USAGE_SW_WRITE_RARELY, bounds, &dstYCbCr));
    } else {
        CHECK_EQ(0, mapper.lock(buf->handle,  GRALLOC_USAGE_SW_READ_NEVER | GRALLOC_USAGE_SW_WRITE_RARELY, bounds, &dst));
    }
    ...
    if (mConverter) {
        ...
        uint8_t *dst_y = (uint8_t *)(dstYCbCr.y);
        size_t dst_y_size = dstYCbCr.ystride * mHeight;
        size_t dst_c_stride = dstYCbCr.cstride;
        size_t dst_c_size = dst_c_stride * mHeight / 2;
        uint8_t *dst_v = (uint8_t *)(dstYCbCr.cr);
        uint8_t *dst_u = (uint8_t *)(dstYCbCr.cb);
